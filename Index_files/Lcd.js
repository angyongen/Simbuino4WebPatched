$(function () {
	...
	Lcd =
	{
		...
		UpdateImagePixel : function(x, y)
		{
			var bR = this.LcdCurrentBacklight.R;
			var bG = this.LcdCurrentBacklight.G;
			var bB = this.LcdCurrentBacklight.B;
			var fR = this.LcdForeground.R;
			var fG = this.LcdForeground.G;
			var fB = this.LcdForeground.B;
			if (this.Pixels[y * this.Width + x]) {
			    this.Context.fillStyle = `rgb(${fR}, ${fG}, ${fB})`;
			} else {
			    this.Context.fillStyle = `rgb(${bR}, ${bG}, ${bB})`;
			}
		...
		{						
			...
			var foreB = this.LcdForeground.B;
			var pixels = this.Pixels;
			var num_pixels = this.Width * this.Height;//84 * 48;
			var src = 0;
			var dst = 0;
			var data = this.Image.data;
			for (var i = 0; i < num_pixels; i++)
